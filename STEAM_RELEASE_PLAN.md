...
> 当前仓库定位为 **Steam 级垂直切片 + Desktop Foundation**，不代表 Steamworks、Deck Verified 或 Steam 1.0 已完成。
...
### 已完成：Blender 炮台 V2 与资产预算管线

- 炮台新增黏液弹药罐、充能线圈、炮口发光环、压力表指针、状态灯和安全底环，Three.js 会按蓄力、库存、开火和任务稳定度驱动这些节点。
- 导出管线会清理退化面和无用顶点属性、按运行时节点合批、去重、应用 Meshopt，再执行 Khronos 与自定义契约/预算验证。
- 当前发布 GLB 为 8 个网格、11 个图元、27,336 个三角面、4 个材质和 138,576 字节；资产清单记录 MIT 许可、工具链、SHA-256 与预算数据。
...
- `src/main.js` 仍集中了较多场景、玩法、物理、音频和 UI 职责；固定 60 Hz 模拟、状态机拆分与实体生命周期仍需完成。
- 炮弹、粒子、动物、无人机和污渍的对象池，以及画质预设、LOD、Instancing、阴影/粒子分级和 WebGL Context Lost 恢复尚未完成。
- 当前只完成炮台的 Blender 发布资产；动物、无人机、Boss 与环境仍需模型化、动画化、LOD 化与许可清单化。
- 分辨率/显示器/窗口模式、完整输入重映射、中英文本地化、正式音乐与辅助功能审核仍属发行工作。

### 下一阶段实施顺序

1. 先建立对象池、性能计数器和长时间压力测试，消除炮弹、粒子、目标、无人机与污渍的频繁分配。
2. 完成低/中/高画质预设、动态渲染比例、阴影与粒子分级、LOD/Instancing，并在 Steam Deck 1280×800 和最低配置 PC 上采集基线。
...
- 固定时间步、对象池、画质预设、LOD 与两小时性能/内存压力测试尚未完成。
